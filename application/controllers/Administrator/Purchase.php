...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        $purchases = $this->db->query("
            ...
            where pm.status <> 'd'
            and pm.PurchaseMaster_BranchID = '$branchId' 
    ...
    public function purchase_pending_record()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Purchase Pending Record";
        $data['content'] = $this->load->view('Administrator/purchase/purchase_pending_record', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
...
<?php
    ...
    public function adjustment_entry()
    {
        ...
        $data['title'] = "Adjustment Entry";
        $data['adjustmentCode'] = $this->mt->generateAdjustmentCode();
        $data['content'] = $this->load->view('Administrator/purchase/adjustment_entry', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addAdjustment()
    {
        ...
        try {
            ...
            $res = ['success' => true, 'message' => 'Adjustment entry success', 'newCode' => $this->mt->generateAdjustmentCode()];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function updateAdjustment()
    {
        ...
        try {
            ...
            $res = ['success' => true, 'message' => 'Adjustment updated successfully', 'newCode' => $this->mt->generateAdjustmentCode()];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function deleteAdjustment()
    {
        ...
        try {
            ...
            $this->db->where('Adjustment_SlNo', $adjustmentId)->update('tbl_adjustment', ['status' => 'd']);
            $this->db->where('Adjustment_SlNo', $adjustmentId)->update('tbl_adjustmentdetails', ['status' => 'd']);

            $res = ['success' => true, 'message' => 'Adjustment deleted successfully', 'newCode' => $this->mt->generateAdjustmentCode()];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function getAdjustments()
    {
        ...
        $clauses = "";
        if (isset($data->adjustmentId) && $data->adjustmentId != '') {
            $clauses .= " and d.Product_SlNo = '$data->adjustmentId'";
        }
        $adjustments = $this->db->query("
            select
                dd.Product_SlNo,
                dd.AdjustmentDetails_AdjustmentQuantity,
                dd.adjustment_rate,
    ...
    public function adjustment_product_list()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Product Adjustment List";
        $data['products'] = $this->db->query("select * from tbl_product p where p.status = 'a' and p.is_service = 'false'")->result();
        $data['content'] = $this->load->view('Administrator/purchase/adjustment_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    function adjustment_select_product()
    {
        $prod_id = $this->input->post('prod_id');
        $clauses = "";
        if ($prod_id != 'All' && !empty($prod_id)) {
            $clauses .= " AND p.Product_SlNo = '$prod_id'";
        }
        $query = $this->db->query("SELECT
                                    ...
                                    p.Product_Name,
                                    p.Unit_ID,
                                    concat(p.Product_Code,'-',p.Product_Name) as productDetail,
                                    un.Unit_Name
                                FROM tbl_adjustmentdetails ad
        ...
        $dueResult = $this->db->query("
            select 
                (
                    ...
                    where sp.Purchase_id = pm.PurchaseMaster_SlNo
                    and sp.SPayment_status = 'a'
                ) as paid,
                (
                    select pm.PurchaseMaster_DueAmount - paid
                ) as due
    ...
    public function GetPurchaseInvoices()
    {
        ...
        $invoices = $this->db->query("
            ...
            where PurchaseMaster_DueAmount != payment_amount
            and Supplier_SlNo = ?
            order by PurchaseMaster_SlNo desc
        ", [$branchId, $data->supplierId])->result();

        echo json_encode($invoices);
    }

    public function exportExcelPurchaseRecord($dateFrom, $dateTo)
    {
        ...
        $purchases = $this->db->query("
            ...
            from tbl_purchasemaster pm
            join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
            where pm.PurchaseMaster_BranchID = '$this->brunch' 
            and pm.status = 'a'
        ...
        $sheet->setCellValue('G1', 'Transport Cost');
        $sheet->setCellValue('H1', 'Total');
        $sheet->setCellValue('I1', 'Paid');
        $sheet->setCellValue('J1', 'Due');
        $sheet->setCellValue('K1', 'Note');

        $rows = 2;

        foreach ($purchases as  $val) {
            $sheet->setCellValue('A' . $rows, $val->PurchaseMaster_InvoiceNo);
            $sheet->setCellValue('B' . $rows, $val->PurchaseMaster_OrderDate);
            $sheet->setCellValue('C' . $rows, $val->Supplier_Name);
            $sheet->setCellValue('D' . $rows, $val->PurchaseMaster_SubTotalAmount);
            $sheet->setCellValue('E' . $rows, $val->PurchaseMaster_Tax);
            $sheet->setCellValue('F' . $rows, $val->PurchaseMaster_DiscountAmount);
            $sheet->setCellValue('G' . $rows, $val->PurchaseMaster_Freight);
            $sheet->setCellValue('H' . $rows, $val->PurchaseMaster_TotalAmount);
            $sheet->setCellValue('I' . $rows, $val->PurchaseMaster_PaidAmount);
            $sheet->setCellValue('J' . $rows, $val->PurchaseMaster_DueAmount);
            $sheet->setCellValue('K' . $rows, $val->PurchaseMaster_Description);
            $rows++;
        }
