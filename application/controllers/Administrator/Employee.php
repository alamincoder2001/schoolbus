...
//Start Job Type ------//
    ...
    public function jobType()
    {
        ...
        $data['title'] = "Add jobType";
        $data['content'] = $this->load->view('Administrator/employee/jobType', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function insertJobType()
    {
        $mail = $this->input->post('job_type');
        $query = $this->db->query("SELECT Job_Type from tbl_job_type where Job_Type = '$mail'");
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->mt->save_data('tbl_job_type', $data);
            $message = "Save Successful";
            echo json_encode($message);
        }
    }
    ...
    public function jobTypeEdit($id)
    {
        ...
        $fld = 'jobType_SlNo';
        $data['selected'] = $this->Billing_model->select_by_id('tbl_job_type', $id, $fld);
        $data['content'] = $this->load->view('Administrator/edit/jobtype_edit', $data);
        //$this->load->view('Administrator/index', $data);
    }
    ...
    public function jobTypeUpdate()
    {
        ...
        $this->mt->update_data("tbl_job_type", $data, $id, $fld);
    }
    ...
    public function jobTypeDelete()
    {
        $fld = 'jobType_SlNo';
        $id = $this->input->post('deleted');
        $this->mt->delete_data("tbl_job_type", $id, $fld);
        //$this->load->view('Administrator/ajax/depertment');
    ...
    public function getJobType(){
        ...
        $jobTypes = $this->db->query("
            SELECT 
                jt.*,
                jt.Job_Type 
                from tbl_job_type jt
                where jt.status = 'a'
                ")->result();
...
public function jobLocation()
{
    ...
    $data['title'] = "Add jobType";
    $data['content'] = $this->load->view('Administrator/employee/jobLocation', $data, TRUE);
    $this->load->view('Administrator/index', $data);
}

public function insertJobLocation()
{
    $mail = $this->input->post('job_location');
    $query = $this->db->query("SELECT Job_location from tbl_job_location where Job_Location = '$mail'");
    ...
    if ($query->num_rows() > 0) {
        ...
        $this->mt->save_data('tbl_job_location', $data);
        $message = "Save Successful";
        echo json_encode($message);
    }
}

public function jobLocationEdit($id)
{
    $data['title'] = "Edit Job Type";
    $fld = 'jobLocation_SlNo';
    $data['selected'] = $this->Billing_model->select_by_id('tbl_job_location', $id, $fld);
    $data['content'] = $this->load->view('Administrator/edit/jobLocationEdit', $data);
    //$this->load->view('Administrator/index', $data);
}

public function jobLocationUpdate()
{
    $id = $this->input->post('id');
    $fld = 'jobLocation_SlNo';
    $data = array(
    ...
    $this->mt->update_data("tbl_job_location", $data, $id, $fld);
}

public function jobLocationDelete()
{
    $fld = 'jobLocation_SlNo';
    $id = $this->input->post('deleted');
    $this->mt->delete_data("tbl_job_location", $id, $fld);
    //$this->load->view('Administrator/ajax/depertment');
}
...
public function getJobLocation(){
    ...
    $jobLocations = $this->db->query("
        SELECT 
            jl.*,
            jl.Job_Location
            from tbl_job_location jl
            where jl.status = 'a'
            ")->result();
