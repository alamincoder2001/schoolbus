...
class Customer extends CI_Controller
{
    ...
    public function getCustomers()
    {
        ...
        $customers = $this->db->query("
            ...
            left join tbl_department dp on dp.Department_SlNo = c.department_id
            left join tbl_job_type jt on jt.jobType_SlNo = c.job_type_id
            left join tbl_job_location jl on jl.jobLocation_SlNo = c.job_location_id
            where c.status = 'a'
    ...
    public function getCustomerCreditLimit()
    {
        ...
        $dueResult = $this->mt->customerCreditLimit($clauses);
    ...
    public function customerPaymentPageExcel()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Customer Payment Excel Import";
        $data['content'] = $this->load->view('Administrator/due_report/customerPaymentPageExcel', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function upload_customer_payment_excel()
    {
        $data = json_decode($this->input->raw_input_stream);

        $dataArray = $data->tbl_customer_payment;

        if (count((array)$dataArray[0]) == 5) {
            foreach ($dataArray as $key => $value) {
                ...
                if ($key != 0) {
                    ...
                    $insertData = [
                        ...
                        'CPayment_customerID'      => $value[2],
                        'CPayment_TransactionType' => 'CR',
                        'CPayment_amount'          => $value[3],
                        'CPayment_Paymentby'       => 'Cash',
                        'CPayment_notes'           => $value[4],
                        'CPayment_status'          => 'a',
                    ...
                    $this->db->insert('tbl_customer_payment', $insertData);
                }
            }
            $res = ['success' => true, 'message' => 'Success'];
        } else {
            $res = ['success' => false, 'message' => 'file formate or column problem'];
        }

        echo json_encode($res);
    }

    public function upload_customer_payment_excel_2()
    {
        $data = json_decode($this->input->raw_input_stream);
        // print_r($_FILES);
        ...
        while (($filesop = fgetcsv($handle, 1000, ",")) !== false) {
            ...
            if ($c <> 0) {
                ...
                $cid = $this->db->query("SELECT * FROM tbl_customer WHERE Customer_Code = '$filesop[1]' ")->row();
                $cusId =  $cid->Customer_SlNo;

                $insertData = [
                    'month_id'                 => $monthId,
                    'CPayment_date '           => date('Y-m-d'),
                    'CPayment_invoice'         => $this->mt->generateCustomerPaymentCode(),
                    'CPayment_customerID'      => $cusId,
                    'CPayment_TransactionType' => 'CR',
                    'CPayment_amount'          => $filesop[7],
                    'CPayment_Paymentby'       => 'Cash',
                    'CPayment_notes'           => $filesop[8],
                    'CPayment_status'          => 'a',
                    'CPayment_Addby'           => $this->session->userdata("FullName"),
                    'CPayment_AddDAte'         => date('Y-m-d H:i:s'),
                    'CPayment_brunchid'        => $this->session->userdata("BRANCHid"),
