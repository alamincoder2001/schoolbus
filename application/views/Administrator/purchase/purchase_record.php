...
<div id="purchaseRecord">
    ...
    <div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: purchases.length > 0 ? '' : 'none'}">
        <div class="col-md-12" style="margin-bottom: 10px;display:flex;justify-content: space-between;">
            ...
            <a :href="`${link}${dateFrom}/${dateTo}`" style="margin: 0px;background: #ff9b1fd9;padding: 3px;color: white;text-decoration: none;">Export Excel</a>
        </div>
        <div class="col-md-12">
                        ...
                        <template v-for="purchase in purchases">
                            <tr :style="{background: purchase.status == 'p' ? 'rgb(255 251 220)' : ''}">
                                ...
                                <td style="text-align:center;">
                                    <a href="" title="Purchase Invoice" v-bind:href="`/purchase_invoice_print/${purchase.PurchaseMaster_SlNo}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    <?php if ($this->session->userdata('accountType') != 'u') { ?>
                                        <a v-if="purchase.status == 'p'" href="" title="Approve" @click="approveItem(purchase.PurchaseMaster_SlNo)"><i style="font-size:18px" class="fa fa-check-square"></i></a>
                                        <a href="javascript:" title="Edit Purchase" @click="checkReturnAndEdit(purchase)"><i class="fa fa-edit"></i></a>
                                        <a href="" title="Delete Purchase" @click.prevent="deletePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-trash"></i></a>
                                    <?php } ?>
                                </td>
