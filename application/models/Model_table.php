...
class Model_Table extends CI_Model
{
    ...
    public function generateAdjustmentCode()
    {
        $code = "SA0001";

        $lastAdjustment = $this->db->query("select * from tbl_adjustment order by Adjustment_SlNo desc limit 1");
        if ($lastAdjustment->num_rows() != 0) {
            $newAdjustmentCode = $lastAdjustment->row()->Adjustment_SlNo + 1;
            $zeros = array('0', '00', '000');
            $code = 'SA' . (strlen($newAdjustmentCode) > count($zeros) ? $newAdjustmentCode : $zeros[count($zeros) - strlen($newAdjustmentCode)] . $newAdjustmentCode);
        }
    ...
    public function currentStock($clauses = '', $supplierClause = '')
    {
        $stock = $this->db->query("
            select * from(
                select
                    ci.*,
                    (select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity + ci.adjustment_qnty_add) - (ci.sales_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity + ci.adjustment_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
                    p.supplier_id,
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            c.owner_name,
            c.Employee_ID,
            (select ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00) + ifnull(c.opening_due, 0.00)
    ...
    public function customerCreditLimit($clauses = "", $date = null)
    {
        $branchId = $this->session->userdata('BRANCHid');
        $currentLimit = $this->db->query("
            select 
            c.*,
            (select  ifnull(c.Customer_Credit_Limit, 0.00) - ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00) + ifnull(sum(sr.SaleReturn_ReturnAmount), 0.00)
                from tbl_salesmaster sm 
                left join tbl_salereturn sr on sr.SaleMaster_InvoiceNo  = sm.SaleMaster_InvoiceNo 
                where sm.SalseCustomer_IDNo = c.Customer_SlNo
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                and sm.Status = 'a') as currentCreditLimit
            from tbl_customer c
            where c.Customer_brunchid = '$branchId' $clauses
        ")->result();
    ...
    public function productStock($productId)
    {
        ...
        if ($stockCount != 0) {
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity + $stockRow->adjustment_qnty_add)
                - ($stockRow->sales_quantity + $stockRow->purchase_return_quantity + $stockRow->damage_quantity + $stockRow->transfer_from_quantity + $stockRow->adjustment_quantity);
        }
    ...
    public function getCustomerDueById($Custid)
    {
        ...
        $prevDue = $prevDueAmount->opening_due;
